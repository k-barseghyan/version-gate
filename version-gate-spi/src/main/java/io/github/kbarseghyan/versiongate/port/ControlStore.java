...
public interface ControlStore {
  ...
  /**
   * Atomically allocates a coordinator version and fencing token and creates a candidate build.
   *
   * <p>The adapter may abandon an expired candidate in the same critical section. Fencing tokens
   * and coordinator versions must be monotonically increasing and never reused for a resource.
   * Failed and abandoned builds keep their allocated versions; gaps are valid. For a coordinated
   * resource, the same atomic operation initializes exactly one {@code PENDING} participant state
   ...
   * @return newly created build containing the allocated version and fencing token
   */
  Build beginBuild(String resourceId, String owner, Duration leaseDuration);
